...
ConvexPolygon getIntersection (const ConvexPolygon& polygon,
    ...
    std::vector<geometry::Point> all_point_list = polygon.getAllPoints();
    for (size_t pos = 0; pos < triangle.size(); pos++) {
        all_point_list.push_back(triangle[pos]);
    }
    for (size_t pos_triangle = 0; pos_triangle < triangle.size(); pos_triangle++) {
        Point segment1_a = triangle[pos_triangle];
        Point segment1_b = triangle[ (pos_triangle + 1) % triangle.size() ];
        for (size_t pos_polygon = 0; pos_polygon < polygon.size(); pos_polygon++) {
            Point segment2_a = polygon[pos_polygon];
            Point segment2_b = polygon[ (pos_polygon + 1) % polygon.size() ];
    ...
    for (const geometry::Point& point : all_point_list) {
        if (polygon.isInside(point) && triangle.isInside(point)) {
